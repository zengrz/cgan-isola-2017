#!/usr/bin/env python
# -*- coding: utf-8 -*-
# python main.py --data /media/rey/Data/img_gen/data/facade/train --mode AtoB
...
from CustomDataFlow import CustomDataFlow
...
class Model(GANModelDesc):
    ...
    def build_graph(self, input, output):
        m1_1, v1_1 = tf.nn.moments(input, [1, 2], keep_dims=True, name='moment1_1')
        l = tf.nn.batch_normalization(input, m1_1, v1_1, 0, 1, 1e-5, 'bn1_1')
...
def get_data():
    ...
    ds = CustomDataFlow(datadir, 'train', shuffle=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--infer_folder', action='store_true', help='run sampling')
    ...
    if args.infer_folder:
        assert args.load
        infer_folder(args.data, args.load)
    else:
        logger.auto_set_dir()
